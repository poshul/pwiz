    ...
    public class ManageResultsTest : AbstractFunctionalTest
    {
        ...
        protected override void DoTest()
        {
            ...
            Assert.IsTrue(missingFileMessage.Message.Contains(
                docRename.Settings.MeasuredResults.Chromatograms[0].MSDataFileUris.ToArray()[0].ToString()));
            RunUI(() =>
            {
            ...
            Assert.AreEqual(new MsDataFileLocalUri(TestFilesDir.GetTestPath("160109_Mix1_calcurve_073.mzML")),
                docReimport.Settings.MeasuredResults.Chromatograms[1].MSDataFileUris.ToArray()[0]);
        ...
        private static void CheckResultsEquivalent(SrmDocument docOrig, int revisionInc, bool allowNameChanges)
        {
            ...
            // are preserved.
            using (var enumTranGroupOrig = docOrig.PeptideTransitionGroups.GetEnumerator())
            {
